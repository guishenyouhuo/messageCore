...
<c:if test="${empty messages}">
   ...
   <c:if test="${!empty messages}">
	 ...
	 <form id="form1" name="form1" method="post" action="<%=request.getContextPath() %>/message/showUserTask?pageNow=${pageNow}">
	   <p align="center">
	   <c:if test="${pageNow>1}">
	   <a href="<%=request.getContextPath() %>/message/showUserTask?pageNow=1">第一页</a> || 
	   <a href="<%=request.getContextPath() %>/message/showUserTask?pageNow=${pageNow-1}">上一页</a> ||
	   </c:if>
	   <c:if test="${pageNow<pageCount}">
	    <a href="<%=request.getContextPath() %>/message/showUserTask?pageNow=${pageNow+1}">下一页</a> 
	    || <a href="<%=request.getContextPath() %>/message/showUserTask?pageNow=${pageCount}">最后一页</a> 
	   </c:if>
  ...
  <tr>
    <td><div align="right"><a href="<%=request.getContextPath() %>/message/toModifyHf?id=${message.id}&type=task&pageNow=${pageNow}">修改回访</a></div></td>
  </tr>
</table>
</td>
      </tr>
